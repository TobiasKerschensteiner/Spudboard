...
const int dist = 20000; //Kleiner versatzt von 20000 schritten
...
int minutes = 0;  // Minutenvariable
int seconds = 0;  // Sekundenvariable

 // für ablaufenden Timer 
...
void time() {
  ...
  if (currentMillis - previousMillis >= interval) {  // Überprüfe, ob das festgelegte Intervall vergangen ist
    ...
    // Erhöhe die Sekunden jede Sekunde
    seconds++;
    if (seconds >= 60) {  // Wenn 60 Sekunden erreicht sind, setze Sekunden zurück und erhöhe Minuten
      seconds = 0;
      minutes++;
    }

    if (minutes >= 60) {  // Wenn 60 Minuten erreicht sind, setze Minuten zurück
      minutes = 0;
    }

    // Bereite die Zeit als Zeichenkette vor
    char timeStr[6];
    sprintf(timeStr, "%02d:%02d", minutes, seconds);  // Formatieren der Zeit im Format mm:ss

    tft.fillScreen(TFT_BLACK); // Lösche den Bildschirm
    tft.drawString(timeStr, tft.width() / 2, tft.height() / 2); // Zeichne die Zeit in der Mitte des Bildschirms
  }
}

//Funtion des Display wie es sich im Fahrenmodus verhalten soll
void fahren(){
...
//Funktion wie sich das Display im Lademoud verhalten soll
void laden(){
  static unsigned long lastChangeMillis = 0;
  static int state = 0;
  static unsigned long timerPreviousMillis = 0;
...
//Roboter fährt einen kleinen versatzt von 20000 schritte
...
//Roboter macht vor der Drehung einen kleinen versatzt nach hinten
...
//  int taster2state = digitalRead(taster2); // Lesen Sie den Zustand der Taste
  ...
  switch (currentState) {
    ...
    case MOVING_FORWARD:
      moveForward(); // Bewegt sich vorwärts
      if (sensorValue == HIGH) { // Wenn der Sensor 1 ausgibt
      ...
      case MOVEBACK:
      moveShortDistanceback(bist); //macht einen kleinen versatzt nach hinten
        if (turnLeftNext) {
    ...
    case MOVING_SHORT_DISTANCER:
      moveShortDistance(dist); // Bewegt sich eine kurze Strecke vorwärts
      if (digitalRead(sensorPin) == HIGH) {
        currentState = STOPPINGOR;
      } else {
...
// Motor stoppt sobal er am ende der Tafel angekommen ist und geht ins homecomming system
    case STOPPINGUR:
      stopMotors();
      delay(2000);
      currentState = HOMEUR;
...
//geht in die Funktion homecomming unten rechts
    case HOMEUR:
    homeur();
    break;

//Motoren stoppen wsoabld sie das ende der tafel erreicht haben und gehe ins homecomming system 
    case STOPPINGOR:
      stopMotors();
      delay(2000);
      currentState = HOMEOR;
...
//geht in die Funktion Homecomming oben rechts
    case HOMEOR:
    homeor();
    break;
