...
const int back = 1000; //anzahl der schirtte die er nac hinten fahren soll 
...
enum State {MOVING_FORWARD,
            ...
            STOPPINGUR, HOMEUR, STOPPINGOR, HOMEOR, MOVING_BACK};
...
//Roboter fährt einen kleinen versatzt von 1000 schritte nach hinten bevor er die 90° drehung macht 
void moveShortDistanceback(int steps) {
  ...
  stepper1.move(-steps); //- dafür da das er rückwärts fährt 
  stepper2.move(-steps);
  while (stepper1.distanceToGo() != 0 || stepper2.distanceToGo() != 0) {
    stepper1.run();
    stepper2.run();
...
void loop() {
  ...
  switch (currentState) {
    case MOVING_FORWARD:
      moveForward(); // Bewege dich vorwärts
      if (sensorValue == HIGH) { // Wenn der Sensor 1 ausgibt soll er eine kleine bewegung nach hinten machen und dann seine drehungen machen 
        currentState = MOVING_BACK;
      }
      break;

    case MOVING_BACK:
      moveShortDistanceback(back); // Fährt einen kleine distanz nach hinten 
      if (turnLeftNext) {
        currentState = TURNING_LEFT; //Wechsle den Zustand zu Linksabbiegung
        turnLeftNext = false; //Setze zurück, damit das nöchste abbiegen weider rechts ist 
      } else {
